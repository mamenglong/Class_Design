...
public class RichTextEditor extends ScrollView {
	...
	protected void removeAllView(){
        allLayout.removeAllViewsInLayout();
        LinearLayout.LayoutParams firstEditParam = new LinearLayout.LayoutParams(
                LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
        EditText firstEdit = createEditText("input here",
                dip2px(EDIT_FIRST_PADDING_TOP));
	...
	private EditText createEditText(String hint, int paddingTop) {
		EditText editText = (EditText) inflater.inflate(R.layout.edit_item_new,
				null);
	...
	private void addImageViewAtIndex(final int index, Bitmap bmp,
        ...
        if(getWidth()==0)
            viewWidth=MyApplication.getWidth();
		int imageHeight = viewWidth * bmp.getHeight() / bmp.getWidth();
	...
	public void seeDetial(List<RecyclerViewItem> list){
	    allLayout.removeAllViewsInLayout();
        for (RecyclerViewItem item:list ) {
            if(item.getTitle()!="")
            {
                LinearLayout.LayoutParams firstEditParam = new LinearLayout.LayoutParams(
                        LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
                editNormalPadding = dip2px(EDIT_PADDING);
                EditText firstEdit = createEditText(item.getTitle(),
                        dip2px(EDIT_FIRST_PADDING_TOP));
                firstEdit.setText(item.getTitle());
                allLayout.addView(firstEdit, firstEditParam);
                lastFocusEdit = firstEdit;
            }
            else if(item.getBitmapPath()!=""){
                insertImage(item.getBitmapPath(), MyApplication.getWidth());
            }
        }
